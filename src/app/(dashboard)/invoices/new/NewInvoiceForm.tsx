...
export default function NewInvoiceForm({ org, clients, defaultInvoiceNumber }: NewInvoiceFormProps) {
  ...
  return (
    <div className="w-full max-w-3xl mx-auto">
      <div className="mb-5">
        <Link href="/invoices" className="inline-flex items-center gap-1 text-sm text-zinc-400 hover:text-zinc-700 transition-colors">
          <ChevronLeft size={14} />
          Invoices
        </Link>
      </div>
      ...
      {error && (
        <p className="mb-4 text-sm text-red-600 bg-red-50 px-3 py-2.5 rounded-md">{error}</p>
      )}
      ...
      <div className="space-y-3">
          ...
          <div className="mt-5 pt-4 border-t border-zinc-100 flex justify-end">
            <div className="w-full sm:w-56 space-y-2.5">
              <div className="flex justify-between text-sm text-zinc-500">
                <span>Subtotal</span>
                <span className="text-zinc-900">{formatMoney(subtotal, form.currency)}</span>
              </div>
              ...
              {form.tax_rate > 0 && (
                <div className="flex justify-between text-sm text-zinc-400">
                  <span>{org.tax_name} ({form.tax_rate}%)</span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <textarea value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))}
                rows={3} placeholder="Any additional notes for the client..."
            ...
            <div>
              ...
              <textarea value={form.terms} onChange={e => setForm(f => ({ ...f, terms: e.target.value }))}
                rows={3} placeholder="e.g. Payment due within 30 days."
                className="w-full px-3 py-2 rounded-lg border border-zinc-200 bg-white text-sm text-zinc-900 placeholder-zinc-400 focus:outline-none focus:border-zinc-400 transition-colors resize-none"
              />
            </div>
          </div>
        ...
        <div className="flex gap-2 justify-end">
          <Link href="/invoices" className="inline-flex items-center h-9 px-4 rounded-md border border-zinc-200 text-sm text-zinc-600 hover:bg-zinc-50 transition-colors">
            Cancel
          </Link>
          <button type="button" onClick={() => handleSubmit('draft')} disabled={saving}
            className="h-9 px-4 rounded-md border border-zinc-300 bg-white text-sm font-medium text-zinc-700 hover:bg-zinc-50 disabled:opacity-50 transition-colors">

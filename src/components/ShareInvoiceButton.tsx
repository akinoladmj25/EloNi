...
export default function ShareInvoiceButton({ invoiceId, publicToken: initialToken }: Props) {
  ...
  const disable = async () => {
    const supabase = createClient()
    await supabase.from('invoices').update({ public_token: null }).eq('id', invoiceId)
    setToken(null)
    setOpen(false)
  }
      ...
      {open && token && (
        <>
          ...
          <div className="absolute right-0 top-full mt-2 z-20 w-80 bg-white rounded-lg border border-zinc-200 shadow-lg p-4">
            <p className="text-xs font-semibold text-zinc-700 mb-1">Client portal link</p>
            <p className="text-xs text-zinc-400 mb-3">Anyone with this link can view and print this invoice.</p>
